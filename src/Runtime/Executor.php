...
use Bic\Preprocessor\Directives\Executor as DirectivesExecutor;
use Bic\Preprocessor\Directives\ExecutorInterface;
use Bic\Preprocessor\Directives\RepositoryProviderInterface as Directives;
use Bic\Preprocessor\Exception\NotReadableException;
use Bic\Preprocessor\Exception\PreprocessException;
use Bic\Preprocessor\Exception\PreprocessorException;
use Bic\Preprocessor\Expression\Parser;
use Bic\Preprocessor\IncludeDirectories\RepositoryProviderInterface as IncludeDirectories;
...
final class Executor implements LoggerAwareInterface, LoggerInterface
{
        ...
        foreach ($stream as $token) {
            try {
                switch ($token->getName()) {
                    ...
                    case Lexer::T_BLOCK_COMMENT:
                        yield $this->doRenderComment($token);
                        break;
     ...
     * @return string
     */
    private function doRenderComment(TokenInterface $token): string
    {
        if (! ($this->config & Config::KEEP_COMMENTS) || ! $this->stack->isEnabled()) {
            return '';
        }
